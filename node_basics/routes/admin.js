...
router.get('/add-product',(req,res,next )=> {
    // console.log("In another middleware!");                           //we see that the next app.use is not executed as we need a next() method to allow the request to travel on to the next middleware.
     ...
     res.render('add-product', {pageTitle: 'Add Product', path: '/admin/add-product',pathTitle:'Add Product'})      //2nd key value pair (path) is for defining the path for which the following code will be rendered
 }); 

// /admin/add-product => POST
router.post('/add-product',(req,res,next)=> {                                  //there are two methods get and post.Here we are getting a post data or working with post request thus we need app.post
